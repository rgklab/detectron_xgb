...
def to_xgb_matrix(data, labels, balanced=True, verbose=False):
    if balanced:
        neg, pos = len(labels[labels == 0]), len(labels[labels == 1])
        pos_weight = neg / pos
        if verbose:
            print(f"Positive Samples: {pos}, Negative Samples: {neg}, Pos Weight: {pos_weight}")
        return xgb.DMatrix(data, label=labels, weight=labels * pos_weight + (1 - labels))
